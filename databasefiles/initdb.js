...
async function createTable() {
  try {
    ...
    await dbConn.query(`CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, first_name VARCHAR(100) NOT NULL, last_name VARCHAR(100) NOT NULL, email VARCHAR(250) UNIQUE NOT NULL, phone VARCHAR(20), gender ENUM('male', 'female') NOT NULL, hashed_password VARCHAR(256) UNIQUE NOT NULL, role_id INT NOT NULL, FOREIGN KEY(role_id) REFERENCES roles(id), status ENUM('active', 'inactive', 'suspended') DEFAULT 'active', pics_name VARCHAR(100), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)`);
